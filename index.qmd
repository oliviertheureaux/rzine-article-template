...
Un modèle de mise en page Quarto[^1] est [mis à disposition sur GitHub](https://github.com/rzine-reviews/rzine-article-template).
...
Cela crée un répertoire sur votre machine contenant un fichier Quarto `index.qmd`, prêt à être complété.

[^1]: [https://quarto.org](https://quarto.org)

## Métadonnées

Il n'y a pas d'en-tête (YAML) dans le fichier source de l'article (`index.qmd`). Les métadonnées générales de l'article sont à complétées dans le fichier `_quarto.yml` situé à la racine du répertoire. Renseignez-y le titre, sous-titre, auteur, affiliations, résumé, mots-clés... 
...
L’intégralité des données utilisées doit obligatoirement être :
...
Pour appeler une référence dans le corps du document, utilisez le symbole `@`. Par exemple : @CameronTrivedi2013
